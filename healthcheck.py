...
class WoZaiXiaoYuanPuncher(utils.Data):
    def __init__(self):
        super().__init__(city=os.environ["CITY"], address_recommend=os.environ["ADDRESS_RECOMMEND"])
        # 打卡结果
        self.status_code = 0
        # 登陆接口
    ...
    # 登录
    def login(self):
        username, password = str(os.environ["USERNAME"]), str(
            os.environ["PASSWORD"]
        )
    ...
    def sendNotification(self):
        ...
        if os.environ.get("DD_BOT_ACCESS_TOKEN"):
            ...
            data = {
                "msgtype": "text",
                "text": {
                    "content": f"""⏰ {os.environ["USERNAME"]}的我在校园打卡结果通知\n---------\n打卡项目：健康打卡\n\n打卡情况：{notifyResult}\n\n打卡时间: {notifyTime}"""
                },
            }
            r = requests.post(
